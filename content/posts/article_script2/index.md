+++
date = '2026-01-07T23:51:33+09:00'
title = 'article_script2'
tags = ['2026', '2026-01', 'script', 'shortcode']
draft = false
+++
...
嬉しい機能ですが、マークダウンで記述する性質上、ショートコードを覚えている必要があり、めんどくさいのです。
そこで、またスクリプトを用意しました。
...
```
Youtubeでは、`vクエリパラメータ`の値を参照してHugoに投げるようにしています。  
...
こちらも正規表現を使ってグループ化して`user`と`id`に引き渡しています。  
...
{{< callout type="alert" text="こちらはAlertです">}}

{{< callout type="warning" text="こちらはWarningです">}}

{{< callout type="tip" text="こちらはTipです">}}
...
## さいごに
作ったスクリプトをコードブロックで紹介するときに、ブロック内で`shortcode`として認識されてしまい、どうやってエスケープすればいいかで20分くらい使ってしまいました。  
C言語のコメントアウトと同じようにすればいいという記事を見つけて解決に至りました。  
めでたしめでたし。

[参考リンク: 使用しているテーマ](https://github.com/athul/archie)  

[参考リンク: コードブロックでショートコードをエスケープする](https://yuma.cloud/post/2021/01/21/hugo-escape-shortcodes/)
